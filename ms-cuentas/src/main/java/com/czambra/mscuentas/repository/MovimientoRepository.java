...
public interface MovimientoRepository extends JpaRepository<Movimiento, Long> {
    ...
    @Query(value = "SELECT m FROM Movimiento m WHERE m.cuentaId = :cuentaId",
           countQuery = "SELECT COUNT(m) FROM Movimiento m WHERE m.cuentaId = :cuentaId")
    Page<Movimiento> findByCuentaId(@Param("cuentaId") Long cuentaId, Pageable pageable);
    ...
    @Query(value = "SELECT m FROM Movimiento m WHERE m.cuentaId IN :cuentaIds AND m.fechaMovimiento BETWEEN :fechaInicio AND :fechaFin",
           countQuery = "SELECT COUNT(m) FROM Movimiento m WHERE m.cuentaId IN :cuentaIds AND m.fechaMovimiento BETWEEN :fechaInicio AND :fechaFin")
    Page<Movimiento> findByCuentaIdsAndFechaBetween(
